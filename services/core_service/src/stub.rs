...
use asset_ipc_interface::{IAsset, IPC_SUCCESS, IpcCode, SA_NAME, deserialize_map, serialize_maps};
...
fn reply_handle(code: IpcCode, ret: Result<()>, reply: &mut BorrowedMsgParcel) -> IpcResult<()> {
    ...
    logi!("[INFO]on_remote_request, calling function: {}, result code: {}", code, result);
...
fn on_remote_request(stub: &dyn IAsset, code: u32, data: &BorrowedMsgParcel,
    ...
    logi!("[INFO]on_remote_request, calling function: {}", ipc_code);
    match ipc_code {
