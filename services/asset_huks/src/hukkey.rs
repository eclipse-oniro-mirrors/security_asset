...
use std::ptr::{null_mut, drop_in_place, copy_nonoverlapping};
...
impl SecretKey{
    ...
    pub fn generate(&mut self) -> AssetResult<(Box<HksBlob>,Box<HksParamSet>)>{
        ...
        let ret = InitParamSet(genParamSet, g_genParams004_ptr, 5);
        if ret != HKS_SUCCESS{
        ...
        //         params:__IncompleteArrayField::new()
        //     }
        // );
        // let param_set_in: *const HksParamSet = Box::into_raw(hks_param_set);
        unsafe{
            HksGenerateKey(key_alias, genParamSet as *const HksParamSet, null_mut());
        }
        unsafe{Ok((Box::from_raw(key_alias as *mut HksBlob),Box::from_raw(genParamSet as *mut HksParamSet)))}
    }
    /// delete the hukkey
...
/// init param set
pub fn InitParamSet(param_set:*mut HksParamSet, params:*const HksParam, paramcount:u32) -> HuksErrcode{
    let mut ret: HuksErrcode = HksInitParamSet(param_set);
    if ret != HKS_SUCCESS {
        asset_log_error!("HksInitParamSet failed");
        return ret;
    }
    ret = HksAddParams(param_set, params, paramcount);
    if ret != HKS_SUCCESS {
    ...
    ret = HksBuildParamSet(param_set);
    if ret != HKS_SUCCESS {
