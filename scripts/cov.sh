...
grcov ../services/db_operator/ -s ../services/db_operator --binary-path ../../../../out/generic_generic_arm_64only/hisi_higeneric_newphone_standard/exe.unstripped/tests/unittest/asset/asset_UT_test/asset_db_operator_test -t html --branch --excl-br-line "^\s*((debug_)?assert(_eq|_ne)?!|#\[derive\()" --ignore-not-existing -o ./report
